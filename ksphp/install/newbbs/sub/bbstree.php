...
$GLOBALS['CONF_TREEVIEW'] = array(
    ...
    # Requote (rewritten-quote gag) color
    'C_REQUOTE' => 'dd0',
...
#    $tree = preg_replace("/&lt;font color=([a-zA-Z#0-9]+)&gt;(.+?)&lt;\/font&gt;/","<font color=$1>$2</font>", $tree);
     ...
     */
    function &gentree(&$treemsgs, $parentid, $parentbody = null) {

        # Tree string
        $treeprint = '';
        # 2026-08-03 Gikoneko: 子ノードが「書き換え引用」判定で親本文と
        # 照合するため、このノードの本文（引用処理前）を退避しておく。
        $parentbody_for_children = null;
        ...
        foreach ($treemsgs as $pos => $treemsg) {
            if ($treemsg['POSTID'] == $parentid) {
                ...
                # 削除せず残してハイライトする。親投稿の本文と行単位で照合し、
                # 引用行が全て親に存在する（＝素の引用）なら従来通り全削除、
                # 1行でも親に無い＝改変されていれば、引用ブロックをまるごと
                # 残して .requote で色付けする（改変ネタは前後の文脈が無いと
                # 伝わらないため、差分行だけでなくブロック全体を表示する）。
                $parentbody_for_children = $treemsg['MSG'];
                $isrequote = FALSE;
                if ($parentbody !== null && preg_match("/(^|\r)&gt;/", $treemsg['MSG'])) {
                    ...
                    $parentset = array();
                    foreach (explode("\r", $parentbody) as $pl) {
                        $k = $normqline($pl);
                        if ($k !== '') { $parentset[$k] = TRUE; }
                    }
                    # 引用行ごとに親と照合し、改変行だけ .requote で色付け。
                    # 改変なし行はそのまま残す（文脈保持）。
                    # 1行でも改変があれば $isrequote = TRUE（ブロック全削除を抑止）。
                    $qlines = explode("\r", $treemsg['MSG']);
                    foreach ($qlines as $qi => $ql) {
                        if (preg_match("/^\s*&gt;/", $ql)) {
                            $k = $normqline($ql);
                            if ($k !== '' && !isset($parentset[$k])) {
                                $isrequote = TRUE;
                                $qlines[$qi] = '<span class="requote">' . $ql . '</span>';
                            }
                        }
                    }
                    if ($isrequote) {
                        $treemsg['MSG'] = implode("\r", $qlines);
                    }
                }
                if (!$isrequote) {
                    # Delete quotes
        ...
        foreach ($childids as $idx => $childid) {
            $childtree =& $this->gentree($treemsgs, $childid, $parentbody_for_children);
...
#20200210 Gikoneko: unread pointer fix
    ...
    .requote { color:#{$this->c['C_REQUOTE']}; }
