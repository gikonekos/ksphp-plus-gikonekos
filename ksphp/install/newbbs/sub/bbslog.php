...
class Getlog extends Webapp {
    ...
    function prtloglist() {
        ...
        $dh = opendir($dir);
        if (!$dh) {
            $this->prterror (T('DIRECTORY_OPEN_FAILED'));
        }
        ...
        # Output
        $this->sethttpheader();
        print $this->prthtmlhead ($this->c['BBSTITLE'] . ' ' . T('TITLE_OLDLOG_SEARCH'));
    ...
    function getconditions($filename) {
        ...
        if ($conditions['q']) {
            ...
            if (count($conditions['keywords']) > $this->c['MAXKEYWORDS']) {
                $this->prterror (T('TOO_MANY_KEYWORDS'));
            }
        }
    ...
    function prthtmldownload($filename) {
        ...
        if ($this->c['OLDLOGFMT']) {
            $this->sethttpheader();
            print $this->prthtmlhead ($this->c['BBSTITLE'] . ' ' . T('TITLE_OLDLOG'));
    ...
    function prtoldlog($filename, $conditions = "", $isdownload = FALSE) {
        ...
        if (!$fh) {
            ...
            $this->t->addVar('oldlog_upper', 'UNABLE_TO_OPEN_FILE_TEXT', sprintf(T('UNABLE_TO_OPEN_FILE_TEXT'), $filename));
        ...
        if (!(!$this->c['OLDLOGFMT'] and !$conditions)) {
            ...
            if (!$conditions['showall']) {
                ...
                if (@$conditions['q'] != '') {
                    ...
                    $resultmsg .= sprintf(T('SEARCH_RESULT_FOR_QUERY'), $value);
                }
                if ($resultcount > 0) {
                    $resultmsg .= sprintf(T('SEARCH_RESULTS_COUNT'), $resultcount);
                }
                else {
                    $resultmsg .= T('SEARCH_NO_RESULTS');
                }
                #print $resultmsg;
    ...
    function prttopiclist($filename) {
        ...
        if (!$fh) {
            $this->prterror( sprintf(T('FAILED_TO_OPEN_LOG'), $filename) );
        }
        flock ($fh, 1);
        ...
        $this->sethttpheader();
        print $this->prthtmlhead ($this->c['BBSTITLE'] . ' ' . T('TITLE_TOPIC_LIST') . ' ' . $filename);
    ...
    function prtarchivelist() {
        ...
        $this->sethttpheader();
        print $this->prthtmlhead ($this->c['BBSTITLE'] . ' ' . T('TITLE_OLDLOG_ARCHIVE'));
