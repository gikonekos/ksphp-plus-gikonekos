...
### RC19 (2026/08/12)
* RTL 第一階段；新增阿拉伯語語言檔案（language/arabic.txt，298個鍵）

### RC20 (2026/08/15)
* 新增阿拉伯語安裝程式UI檔案；以兩套體系新增7種語言（西班牙語·法語·德語·印尼語·越南語·他加祿語·印地語）。共15種語言
* 文件檔名穩定化；刪除install/index.html

### RC21 (2026/08/17)
* NG雜湊過濾器（SHA-256、部分比對、星號替換、確認對話框）
* 雜湊前對全形英數字進行半形正規化

詳細內容請參閱 `doc/changelog.txt`。
...
* **install.php的自動重新命名** — `_setup.php`在首次使用後會自動改名，而`install.php`沒有該機制。目前僅有刪除步驟的文件說明。
