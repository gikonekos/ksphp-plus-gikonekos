...
### RC18 추가 수정 (2026/08/08) — 설치 프로그램만, 버전 문자열 변경 없음
...
### RC19 (2026/08/12)
* RTL 1단계; 아랍어 언어 파일 추가 (language/arabic.txt, 298키)

### RC20 (2026/08/15)
* 아랍어 인스톨러 UI 파일; 7개 언어 추가 (스페인어·프랑스어·독일어·인도네시아어·베트남어·타갈로그어·힌디어, 양쪽 세트). 총 15개 언어
* 문서 파일명 안정화; install/index.html 삭제

### RC21 (2026/08/17)
* NG 해시 필터 (SHA-256, 부분 일치, 별표 치환, 확인 다이얼로그)
* 해싱 전 전각 영숫자 정규화

자세한 내용은 `doc/changelog.txt` 를 참조하세요.

## 할 일
* **업로더 썸네일 JS** — upthumb.js를 Uploader@Heyuri 이외의 Uploader 인스턴스에도 쉽게 대응할 수 있도록 정비
* **새 게시물 화면에 폼이 표시되지 않음** — 간헐적 발생, 재현 조건 불명
* **실제 브라우저에서의 RTL 표시 확인** — 논리 속성과 `dir` 속성은 도입했으나 실제 표시는 미확인. `.msgtree`가 `white-space: pre`이므로 트리 표시가 가장 깨지기 쉽다. 게시 폼, 헤더 내비게이션 줄, 인용·글 번호 위치도 확인 필요.
* **설치 프로그램용 아랍어 파일** — 게시판 본체의 `language/arabic.txt`는 작성 완료. `install/language/`는 247개 키의 별도 체계이며 아랍어판은 미착수. 설치 프로그램은 언어 목록을 디렉터리 스캔으로 만들기 때문에 현재는 드롭다운에 나타나지 않을 뿐이다.
* **추가 7개 언어 파일** — 스페인어·프랑스어·독일어·인도네시아어·베트남어·타갈로그어·힌디어. 모두 같은 테이블에 등록되어 있으며 `.txt` 본체만 남았다.
* **install.php의 자기 이름 변경** — `_setup.php`는 최초 사용 후 스스로 이름을 바꾸지만 `install.php`에는 이 구조가 없다. 현재는 삭제 절차의 문서화에 그친다.
