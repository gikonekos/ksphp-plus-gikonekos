...
### 2021/07/03
* Added Motoi(gikonekos)'s 2chtrip(20210625)
* Removed bbs.cgi

### 2021/07/27
* Fixed an issue where the admin password leaked when using both admin password and trip (Motoi(gikonekos))
...
* Applied Motoi(gikonekos)'s fixed handle name password prevention (20210923)
...
* Commented out Motoi(gikonekos)'s YouTube embedding function
...
### 2026/03/10
* Administrator keys can now be combined with a tripcode, implemented by Motoi(gikonekos)
...
* Not yet started: full 7-language translation of each conf.php key's
  ...
  **(2026-08-02 update) Done, including a bug fix found along the
  way.** The comment-splitting logic misattributed help text by one
  key for 6 conf.php settings that use same-line trailing comments
  instead of leading comment blocks (C_A_COLOR/C_A_VISITED/
  C_A_ACTIVE/C_A_HOVER/C_SUBJ/C_ERROR -- the link/title/error color
  settings); fixed via new `ksphp_conf_entry_trailing_comment()` /
  `ksphp_conf_build_help_texts()` helpers. On top of that, added
  98 `CONF_HELP_<KEY>` entries to each of the 7
  `install/language/*.txt` files, with a `ksphp_conf_help_text()`
  lookup that falls back to the (now bug-fixed) raw conf.php comment
  extraction for any key without a translation yet. Verified
  end-to-end via PHP's built-in server against the real
  `?ajax=1&action=conf_review` endpoint for all 7 languages. Not yet
  packaged into a new release zip or reflected in the changelog.
