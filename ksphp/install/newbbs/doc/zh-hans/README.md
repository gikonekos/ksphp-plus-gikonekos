...
* **阿拉伯语语言文件** — 仅缺少`language/arabic.txt`；`bbs.php`和`install.php`的语言表中已登记`arabic`及其显示名称，因此只要放入该文件即可启用RTL。
* **另外七个语言文件** — 西班牙语、法语、德语、印尼语、越南语、他加禄语和印地语。它们都已预先登记在同样的表中，仅剩`.txt`文件本体。
* **install.php的自动重命名** — `_setup.php`在首次使用后会自动改名，而`install.php`没有该机制。目前仅有删除步骤的文档说明，以及用于抑制目录列表的`index.html`。
